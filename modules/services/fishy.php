<?PHP
	class fishy {
		...
		function construct() {
				$this->channels = array ( '#sopaintball' => 1 );
		...
		function destruct() {
			$ircd = &ircd();
			$ircd->quit('fishy', 'Module Unloaded.');
		}
			...
			if (isset($this->channels[strtolower($to)])) {
				...
				foreach ($this->cresponses as $regex => $mtpl) {
					if (preg_match($regex,$message,$m)) {
						$ircd->msg('fishy',$to,str_replace(array('%n','%c','%1'),array($from,$to,(isset($m[1]) ? $m[1] : '')),$mtpl));
						break;
					}
				}
			}
			...
			if (isset($this->channels[strtolower($to)])) {
				if (strtolower($type) == 'action') {
					...
					foreach ($this->aresponses as $regex => $mtpl) {
						if (preg_match($regex,$message,$m)) {
							$ircd->msg('fishy',$to,str_replace(array('%n','%c','%1'),array($from,$to,$m[1]),$mtpl));
							break;
						}
					}
				}
...
*/
		function event_eos () {
			...
			$ircd->addnick($mysql->getsetting('server'),'fishy','fish','go.moo.oh.yes.they.do','Teh f1shy <'.chr(176).')))))><');
	...
	function registerm () {
		$class = new fishy;
		register($class, __FILE__, 'Fishy Module', 'fishy');
	}
?>
