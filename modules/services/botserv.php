...
//			$ircd->mode($bot['nick'],'#bots','+v '.$bot['nick']); // Please don't
		...
		function sandbot ($nick) {
			$basedir = dirname(__FILE__).'/botcontroller/'.$nick.'/';
			if(!is_dir($basedir))
				mkdir($basedir);
			$options = array(
				'safe_mode'=>false,
			...
			$this->bots[$nick]['sandbox']->ini_set('error_log',$basedir.'error.log');
			...
			$this->bots[$nick]['sandbox']->eval('chdir(\''.$basedir.'\');');
		}
		...
		function sboteval ($nick,$cmd,$message,$chan,$bot,$script,$othervars) {
			...
			try {
				...
				if(file_exists(dirname(__FILE__).'/botcontroller/bots.ser'))
					$data = unserialize(file_get_contents(dirname(__FILE__).'/botcontroller/bots.ser'));
				else
					$data = array();
				$data[] = array($ubot,$nick,$cmd,$message,$chan,$bot,$script,$othervars,$this->bots[$ubot]['sandbox']->__return__);
				file_put_contents(dirname(__FILE__).'/botcontroller/bots.ser',serialize($data));
			}
			catch (Exception $e) {
				...
				if (isset($data['call'])) {
					if ($called < 3) {
						if ($x = $mysql->get($mysql->sql('SELECT * FROM `botserv_cmds` WHERE `botid` = '.$mysql->escape($this->bots[$ubot]['botid']).' AND `cmd` = '.$mysql->escape($data['call'])))) {
							...
							unset($this->bots[$ubot]['sandbox']->__return__);
							if(file_exists(dirname(__FILE__).'/botcontroller/bots.ser'))
								$botdata = unserialize(file_get_contents(dirname(__FILE__).'/botcontroller/bots.ser'));
							else
								$botdata = array();
							$botdata[] = array($ubot,$nick,$cmd,$message,$chan,$bot,$script,$othervars,$data);
							file_put_contents(dirname(__FILE__).'/botcontroller/bots.ser',serialize($botdata));
							$this->curbot = '';
							$repeat = 1;
						}
					}
		...
		function event_signal_chld() {
			...
			$botdata = unserialize(file_get_contents(dirname(__FILE__).'/botcontroller/bots.ser'));
			unlink(dirname(__FILE__).'/botcontroller/bots.ser');
