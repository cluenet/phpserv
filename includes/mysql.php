<?PHP
	class Database {
		...
		public static function setAccountProperty( $uid, $key, $value, $visibility = 'hidden', $section = 'core' ) {
			$data = array( 
				'uid' => $uid,
				'key' => $key,
				'value' => $value,
			...
			self::insert( 'access_properties', $data, true );
		}
	
		public static function listAccountProperties( $uid = null, $key = null, $value = null, $visibility = null, $section = null ) {
			
			$sql = 'SELECT * FROM `access_properties` WHERE 1=1';
			
			if( $uid !== null )
				$sql .= ' AND `uid` = ' . self::escape( $uid );
			
			if( $key !== null )
				$sql .= ' AND `key` = ' . self::escape( $key );
			
			if( $value !== null )
				$sql .= ' AND `value` = ' . self::escape( $value );
			
			if( $visibility !== null )
				$sql .= ' AND `visibility` = ' . self::escape( $visibility );
			
			if( $section !== null )
				$sql .= ' AND `section` = ' . self::escape( $section );
			
			$result = self::sql( $sql );
			$results = array();
			
			while( $results[] = self::get( $result ) )
				;
			
			return $results;
		}
	
		public static function getAccountProperty( $uid, $key, $section = 'core' ) {
			$data = self::listaccountproperties( $uid, $key, null, null, $section );
			return $data[ 0 ];
		}
	
		public static function delAccountProperty( $uid = null, $key = null, $value = null, $visibility = null, $section = null ) {
